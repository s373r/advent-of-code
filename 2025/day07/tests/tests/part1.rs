...
fn solution(input: &str) -> usize {
    ...
    let manifold_idx = lines_iter.next().unwrap().find('S').unwrap();

    let mut beam_idxs = HashSet::from([manifold_idx]);
    let mut res = 0;

    for line in lines_iter {
        let mut stopped_beam_idxs = HashSet::new();
        let mut new_beam_idxs = HashSet::new();
        ...
        for beam_idx in &beam_idxs {
            ...
            match c {
                ...
                '^' => {
                    ...
                    new_beam_idxs.insert(*beam_idx - 1);
                    new_beam_idxs.insert(*beam_idx + 1);
                }
                _ => unreachable!(),
            }
        ...
        for idx in new_beam_idxs {
            beam_idxs.insert(idx);
        }
    }

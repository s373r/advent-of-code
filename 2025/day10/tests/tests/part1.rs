...
fn solution(input: &str) -> usize {
    input.lines().fold(0, |acc, line| {
        ...
        let indicator_light_diagram = line_chunk_iter
            .next()
            .unwrap()
            .trim_matches(|c| matches!(c, '[' | ']'))
            .chars()
            .collect::<Vec<_>>();
        let _ignored_joltage_requirements = line_chunk_iter.next_back().unwrap();
        ...
        const MAX_BUTTON_PRESSES: usize = 8;

        let mut fewest_button_presses = usize::MAX;
        let mut indices_store = Vec::with_capacity(MAX_BUTTON_PRESSES);
        let mut machine = vec!['.'; indicator_light_diagram.len()];
        ...
        nested_loop(
            ...
            &mut |indices| {
                ...
                if button_presses > fewest_button_presses {
                    // There is no point in performing further recursive calculations of this direction,
                    // since we already have a better option.
                    return false;
                }

                // The machine has the number of indicator lights shown,
                // but its indicator lights are all initially off (c)
                machine.fill('.');

                // Press buttons
                for idx in indices {
                    for button in &button_wiring_schematics[*idx] {
                        let old_light_state = machine[*button];
                        let new_light_state = match old_light_state {
                            '.' => '#',
                            '#' => '.',
                            _ => unreachable!(),
                        };
                        machine[*button] = new_light_state;
                    }
                }

                // Is that acceptable?
                if machine == indicator_light_diagram {
                    if fewest_button_presses > button_presses {
                        fewest_button_presses = button_presses;
                    }

...
// a group of Senior Historians has asked you to accompany them as they check the places they think
...
// *must* be in one of the first fifty places they'll look, so in order to save Christmas, you need to help them get
...
// This seems to be the planning the Chief Historian was doing before he left. Perhaps these notes
...
// but by a unique number called the *location ID*. To make sure they don't miss anything, The Historians split
...
pub fn total_distance_between_lists(input: &str) -> u64 {
    let lines_count = input.lines().count();

    let mut left_list = Vec::with_capacity(lines_count);
    let mut right_list = Vec::with_capacity(lines_count);
...
mod tests {
    ...
    fn test_example() {
        ...
        assert_eq!(11, total_distance_between_lists(input));
    }
    ...
    fn test_input() {
        ...
        assert_eq!(1_258_579, total_distance_between_lists(&input));
    }
}
